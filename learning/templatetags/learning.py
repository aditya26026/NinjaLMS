...
from learning.forms import CourseCollaboratorUpdateRoleForm, ResourceCollaboratorUpdateRoleForm, \
    ActivityCollaboratorUpdateRoleForm
from learning.models import CollaboratorRole, CourseAccess, ResourceType, CourseCollaborator, ActivityAccess, \
    ...
    ResourceCollaborator, ObjectCollaboratorMixin
...
@register.simple_tag
def get_permission_icon(value, related_object):  # pragma: no cover
    button_template = """
    ...
    if value == "view_{related_object}".format(related_object=related_object_name):
        filled_button = format_html(
            button_template, icon="eye", btn_class="primary",
            btn_tooltip=_("Can view the %(related_object)s") % {'related_object': related_object_name}
        )
    elif value == "change_{related_object}".format(related_object=related_object_name):
        filled_button = format_html(
            button_template, icon="pen", btn_class="warning",
            btn_tooltip=_("Change change %(related_object)s") % {'related_object': related_object_name}
        )
    elif value == "delete_{related_object}".format(related_object=related_object_name):
        filled_button = format_html(
            button_template, icon="trash", btn_class="danger",
            btn_tooltip=_("Can delete the %(related_object)s") % {'related_object': related_object_name}
        )
